...
class GoogleJobsSpider(scrapy.Spider):
    ...
    def __init__(self,job_id=None, *args, **kwargs):
        ...
        if max_pages is not None:
            ...
            self.max_pages = None  # No limit on the number of pages if max_pages is None
        self.page_count = 0
    ...
    async def parse(self, response):
        ...
        # Increment the page count and check if it exceeds the max_pages (if max_pages is set)
        self.page_count += 1
        if next_page and (self.max_pages is None or self.page_count < self.max_pages):
            self.logger.info("Navigating to next page: %s", next_page)
            yield response.follow(next_page, callback=self.parse)
        else:
            self.logger.info("Reached max pages limit or no more pages to navigate.")
